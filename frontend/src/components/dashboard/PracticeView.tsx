...
function Story({ practice, session }: { practice: PracticeSummary; session: RaceSession }) {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader title={<span className="flex items-center gap-2"><Sparkles size={15} className="text-accent-soft" /> What we learned in {session.session_type}</span>} />
      ...
      {/* same identity language as the race key cards: portrait + full name */}
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <PCard icon={<Gauge size={15} />} tone="accent" label="Fastest"
          value={nameOf(p.fastest_driver)} avatar={<DriverAvatar driver={driverOf(session, p.fastest_driver)} size={34} />}
          sub={fmtLap(p.fastest_lap)} why="Quickest single lap of the session." />
        <PCard icon={<TrendingUp size={15} />} tone="speed" label="Best long run"
          value={nameOf(p.best_long_run_driver)} avatar={<DriverAvatar driver={driverOf(session, p.best_long_run_driver)} size={34} />}
          sub={<>strongest <Term>long run</Term></>} why="Best read on race pace, on higher fuel." />
        <PCard icon={<Repeat size={15} />} tone="default" label="Most laps"
          value={nameOf(p.most_laps_driver)} avatar={<DriverAvatar driver={driverOf(session, p.most_laps_driver)} size={34} />}
        ...
        <PCard icon={<Clock size={15} />} tone="amber" label="Most improved"
          value={nameOf(p.most_improved_driver)} avatar={<DriverAvatar driver={driverOf(session, p.most_improved_driver)} size={34} />}
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        ...
        <CardBody className="space-y-2">
          {rows.map((r) => (
            <BarRow key={r.driver} session={session} row={r} value={fmtLap(r.best_lap)}
      ...
      <Card>
        ...
        <CardBody className="space-y-2">
          {longs.length ? longs.map((r) => (
            <BarRow key={r.driver} session={session} row={r} value={`${fmtLap(r.long_run_pace)} · ${r.long_run_laps}L`}
...
/* identical structure to the race story's KeyCard: tone colours the icon,
   the driver's name is always white next to their portrait */
function PCard({ icon, label, value, sub, why, tone, avatar }: any) {
...
function BarRow({ session, row, value, pct }: {
  session: RaceSession; row: PracticeSummary["rows"][number]; value: string; pct: number;
}) {
  return (
    <div className="flex items-center gap-2">
      <DriverBadge driver={driverOf(session, row.driver)} code={row.driver}
        name={row.name} team={row.team} teamColor={row.team_color}
        size={24} className="w-36 shrink-0" />
      <span className="h-2 flex-1 overflow-hidden rounded-full bg-white/[0.06]">
        <span className="block h-full rounded-full" style={{ width: `${Math.max(6, pct)}%`, background: row.team_color }} />
      </span>
      <span className="w-28 shrink-0 text-right tabular-nums text-xs text-ink-muted">{value}</span>
    </div>
  );
}
